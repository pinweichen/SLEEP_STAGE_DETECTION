...
if (personalized == T) {
  result_p <- paste0(general,"/result_original_within_sub")
  #subj <- list("001", "003", "004", "005", "007", "008", "009", "010")
  subj <- list("001", "003", "004", "005", "007", "008",  "010") # LDA only
} else {
  result_p <- paste0(general,"/result_original")
  subj <- list("001", "003", "004", "005", "006", "007", "008", "009", "010")
}

#result_p <- paste0(general,"/result_original_record_wise")

#result_p <- paste0(general,"/result_original_acti")
setwd(result_p)
  ...
  for (p in 1:length(stg_ls)){
    ...
    for (q in 1:length(model_ls)){
      ...
      for (n in 1:length(subj)) {
        sub <- subj[[n]]
        dt <- fread(paste0(subf_p,"/pred_actual_SSV_",sub,"_skf.csv"))
      ...
      full_cm_d$Perc <- round(full_cm_p$Freq*100,2)
      ...
      full_cm_d_p <-  ggplot(data = full_cm_d, aes( x =  Reference, y = Prediction , fill = Perc))+
        ...
        ylim(rev(levels(full_cm_d$Prediction))) + xlab("Truth") + ylab("Predicted")
      ...
      for (p in 1:length(stg_ls)){
        ...
        for (q in 1:length(model_ls)){
          ...
          full_cm_d_p <-  ggplot(data = full_cm_d, aes( x =  Reference, y = Prediction , fill = Perc))+
            ...
            #scale_fill_viridis(discrete=FALSE) +
            scale_fill_distiller(palette="Greens", direction=1,limits = c(0,100)) +
            ylim(rev(levels(cm_d$Prediction))) + xlab("Truth") + ylab("Predicted")
          #scale_fill_gradient(low="white", high="Black",limits = c(0,100))
...
ggplot(data=sub, aes(x=time, y=Actual, group = 1), color = "blue") +
  geom_line() + geom_line(data=sub, aes(x=time, y=Predicted, group = 1), color = "red") +
  xlab("Time (min)") + ylab("Sleep Stages") + xlim(0,390) 
#ggsave(paste0(result_p,"/","Hypnogram_",unique(sub$sub),".png"), last_plot(), width = max(b)*3*10, height = 5000, units = "px")
